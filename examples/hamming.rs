...
fn main() {
    ...
    let image1_binary: Vec<u8> = hash1.as_bytes().iter().map(|x| to_binary(*x)).flatten().map(|x| x as u8).collect();
    let image2_binary: Vec<u8> = hash2.as_bytes().iter().map(|x| to_binary(*x)).flatten().map(|x| x as u8).collect();
